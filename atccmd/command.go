...
func (cmd *ATCCommand) constructMembers(
	...
	*/
	...
	/*
		logger.Debug("nomad-worker-provider-enabled")
		workerProvider := &nomadatc.WorkerProvider{Logger: logger}
		err = workerProvider.Init()
		if err != nil {
			return nil, err
		}
	*/
	...
	*/
	...
	members = append(members, grouper.Member{"nomad-worker", workerProvider})
	members = append(members, grouper.Member{"nomad-driver", nomadDriver})
...
func (cmd *ATCCommand) Runner(positionalArguments []string) (ifrit.Runner, error) {
	...
	apiMembers, err := cmd.constructMembers(nomadDriver, positionalArguments, []string{
		"debug",
		"web-tls",
		"web",
	},
	...
	serviceMembers, err := cmd.constructMembers(nomadDriver, positionalArguments, []string{
		"drainer",
		"pipelines",
		"builds",
		"collector",
